...
const CreatorHomeScreen = ({ onNavigate }) => {
  ...
  useEffect(() => {
    ...
    const fetchOrganisationName = async () => {
      try {
        const response = await axios.get(`https://theserver-tp6r.onrender.com/user/getuserorganisation/${username}`);
  ...
  useEffect(() => {
    ...
    const fetchPropertyData = async () => {
      setLoading(true);
      try {
        const response = await axios.get(`https://theserver-tp6r.onrender.com/organisation/organisation/${organizationName}`);
        if (response.status === 200) {
